...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if self.num_classes is not None:
            assert y.shape == (x.shape[0],)
            cemb = self.label_emb(y)
            cemb[torch.where(y == torch.tensor(self.num_classes-1))[0]] = torch.zeros_like(cemb[0])

            # Additional Cemb layer
            cemb = self.cemb_layer(cemb)
...
class Unet_encoder2(nn.Module):
    def __init__(self, in_ch=3, mod_ch=64, out_ch=3, ch_mul=[1,2,4,8], num_res_blocks=2, use_conv=True, droprate=0, dtype=torch.float32):
        ...
        for i, mul in enumerate(self.ch_mul):
            ...
            for _ in range(self.num_res_blocks):
                layers = [
                    ResBlock_b(now_ch, nxt_ch, self.droprate),
                    AttnBlock(nxt_ch)
                ]
                now_ch = nxt_ch
        ...
        self.middleblocks = nn.Sequential(
            ResBlock_b(now_ch, now_ch, self.droprate),
            AttnBlock(now_ch),
            ResBlock_b(now_ch, now_ch, self.droprate)
        )
    ...
    def forward(self, x:torch.Tensor, get_rep=False):
        h = x.type(self.dtype)
