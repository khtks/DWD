...
class nnPUSBloss(nn.Module):
    ...
    def forward(self, out, pu_target):
        ...
        out = torch.clamp(out, min=0.01, max=0.99)
        ...
        if self.IS:
            coeff = torch.tensor([1. if i in p_indices else 5 * self.IS_fn(out[i]) for i in range(len(out))]).cuda()
        else:
...
class nnPUloss(nn.Module):
    ...
    def forward(self, out, pu_target):
        ...
        if self.IS:
            coeff = torch.tensor([1. if i in p_indices else 5 * self.IS_fn(self.loss_fn(-out[i])) for i in range(len(out))]).cuda()
        else:
            coeff = torch.tensor([1. if i in p_indices else self.u_fn(self.loss_fn(-out[i])) for i in range(len(out))]).cuda()
        # coeff = torch.tensor([self.l_fn(out[i]) if i in p_indices else self.u_fn(out[i]) for i in range(len(out))]).cuda()
