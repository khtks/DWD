...
class NoiseScheduleVP:
    def __init__(self, schedule='linear', depth=999):
        ...
        # self.cosine_beta_max = cosine_max
...
class DPM_Solver:
    ...
    def sample(self, x, steps=10, eps=1e-4, T=None, order=3, skip_type='logSNR',
               adaptive_step_size=False, fast_version=True, atol=0.0078, rtol=0.05, start_t=None, get_list=False
               ):
        """
        ...
        if adaptive_step_size:
            ...
            if fast_version:
                if isinstance(t_T, list):
                    timesteps = torch.FloatTensor().cuda()
                    orders = None
                    for s_T in t_T:
                        # print(s_T)
                        # input()
                        #
                        # s_T = s_T[0]
                        order, t_steps = self.get_time_steps_for_dpm_solver_fast(t_T=s_T, t_0=t_0, steps=steps, device=device)
                        timesteps = torch.cat((timesteps, t_steps.reshape(1, -1)), dim=0)
                        orders = order
                else:
            ...
            with torch.no_grad():
                if isinstance(t_T, list): timesteps = timesteps.T
                # print(orders, timesteps)

                d = len(orders)//10
                target_idx = [d, 2*d, 3*d, 5*d, 7*d]
                x_list = []

                #print(orders, len(timesteps))
