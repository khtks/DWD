...
class SDE(object):
    ...
    def sample(self, x0, t_init=0, t=None):  # sample from q(xn|x0), where n is uniform
        if t is None:
    ...
    def xt_sample(self, x0, t, noise=None):  # sample from q(xn|x0), where n is uniform
        mean, std = self.marginal_prob(x0, t)
        if noise==None:
            eps = torch.randn_like(x0)
        else:
...
class ScoreModel(object):
    ...
    def predict(self, xt, t, get_rep=False, **kwargs):
        ...
        return self.nnet(xt, t * 999, get_rep=get_rep, **kwargs)  # follow SDE

    def noise_pred(self, xt, t, get_rep=False, **kwargs):
        if get_rep:
            pred = self.predict(xt, t, get_rep=get_rep, **kwargs)
        else:
...
def LSimple(score_model: ScoreModel, x0, pred='noise_pred', **kwargs):
    ...
    if pred == 'noise_pred':
        ...
        noise_pred = score_model.noise_pred(xt[:(len(xt) // 2)], t[:(len(xt) // 2)], **kwargs)
        return mos(noise - noise_pred)
...
def LSimple_b(score_model: ScoreModel, x0, pred='noise_pred', b=64, **kwargs):
    t, noise, xt = score_model.sde.sample(x0)
    if pred == 'noise_pred':
        noise_pred = score_model.noise_pred(xt, t, **kwargs)
        loss_mse = F.mse_loss(noise_pred, noise, reduction='none')
        loss = (b*torch.mean(loss_mse[:b].squeeze())+7*b*torch.mean((loss_mse[b:]*4).squeeze()))/(b+7*b)
        return loss


def q_sample(score_model: ScoreModel, x0, t, **kwargs):
    return score_model.sde.xt_sample(x0, t)


def LSimple_with_coeff(score_model: ScoreModel, x0, pred='noise_pred', t=None, l_coeff=None, u_coeff=None, lbs=64, indices=None, **kwargs):
    t, noise, xt = score_model.sde.sample(x0, t=t)
    ubs = len(x0) - lbs

    if pred == 'noise_pred':
        noise_pred = score_model.noise_pred(xt, t, **kwargs)

        if l_coeff is None: l_coeff = torch.tensor([1] * lbs).cuda()
        if u_coeff is None: u_coeff = torch.tensor([5] * ubs).cuda()

        len_x = lbs + len(torch.where(u_coeff != 0)[0])
        mse_loss = F.mse_loss(noise_pred, noise, reduction='none')

        l_loss = mse_loss[:lbs].mean([1, 2, 3]) * l_coeff
        u_loss = mse_loss[lbs:].mean([1, 2, 3]) * u_coeff

        loss = (l_loss.sum() + u_loss.sum()) / len_x
...
def LSimple_with_coeff_v2(score_model: ScoreModel, x0, pred='noise_pred', t=None, coeff=None, p_idx=None, u_idx=None, **kwargs):
    t, noise, xt = score_model.sde.sample(x0, t=t)

    if pred == 'noise_pred':
        noise_pred = score_model.noise_pred(xt, t, **kwargs)

        if coeff is None: coeff = torch.tensor([1] * len(x0)).cuda()

        len_x = len(torch.where(coeff != 0)[0])
        mse_loss = F.mse_loss(noise_pred, noise, reduction='none')

        loss = mse_loss.mean([1, 2, 3]) * coeff
        loss = (loss[p_idx].sum() + loss[u_idx].sum()) / len_x
        # loss = (mse_loss[p_idx].sum() + loss[u_idx].sum()) / len_x
        # loss = loss.sum() / len_x
...
def LSimple_with_rep(score_model: ScoreModel, x0, pred='noise_pred', zero_c=False, **kwargs):
    t, noise, xt = score_model.sde.sample(x0)
    ...
    if pred == 'noise_pred':
        ...
        if zero_c:
            c = kwargs['y']
            kwargs = {'y': torch.tensor([max(c)] * len(x0)).cuda()}

        rep = score_model.nnet.get_reps(x=x0, timesteps=t * 999, **kwargs)

        return mse_loss, t, rep
